...
Derive a stable key per verifier from the dongle using the stable-derivation path: `X_v = f(HMAC(K, verifier_id))`. Each verifier sees a distinct key. This breaks cross-verifier correlation on the *key*, but:
